...
const SEED_FILLUPS = [
  { date: '2026-06-20', liters: 42, amount: 1380, fuel: 'gasolina_premium' },
  { date: '2026-06-06', liters: 38, amount: 1216, fuel: 'gasolina_premium' },
  { date: '2026-05-23', liters: 45, amount: 1380, fuel: 'gasolina_premium' },
  { date: '2026-05-09', liters: 40, amount: 1236, fuel: 'gasolina_premium' },
  { date: '2026-04-25', liters: 43, amount: 1344, fuel: 'gasolina_premium' },
];

const VEHICLES = [
  { id: 1, name: 'Mi Auto', make: 'Toyota',  model: 'Corolla', year: 2021, tankSize: 50, fuelType: 'gasolina_premium', level: 0.30 },
];
...
function TankGauge({ level, accentColor }) {
  ...
  const endAngle   = 330;
  const totalArc   = endAngle - startAngle + 360;
  ...
  function arcPath(angle) {
    ...
    const large = (((angle - startAngle + 360) % 360) > 180) ? 1 : 0;
    return `M ${start.x} ${start.y} A ${R} ${R} 0 ${large} 1 ${end.x} ${end.y}`;
  }

  const fullArcAngle = ((endAngle - startAngle + 360) % 360);
  const fillAngle    = startAngle + fullArcAngle * level;
  const trackPath    = arcPath(endAngle < startAngle ? endAngle + 360 : endAngle);
  const fillPath     = level > 0.01 ? arcPath(fillAngle) : null;
  ...
  return (
    <Svg width={180} height={150} viewBox="0 0 180 150">
      {/* Track */}
      <Path
        d={`M ${polarToXY(startAngle, R).x} ${polarToXY(startAngle, R).y} A ${R} ${R} 0 1 1 ${polarToXY(endAngle, R).x} ${polarToXY(endAngle, R).y}`}
        fill="none"
      ...
      {/* Center text */}
      <SvgText x={cx} y={cy - 8} textAnchor="middle" fill={colors.textPrimary}
      ...
      {/* E / F labels */}
      <SvgText x={polarToXY(startAngle, R + 16).x} y={polarToXY(startAngle, R + 16).y + 4}
        textAnchor="middle" fill={colors.textDisabled} fontSize={9}>E</SvgText>
      <SvgText x={polarToXY(endAngle, R + 16).x} y={polarToXY(endAngle, R + 16).y + 4}
        textAnchor="middle" fill={colors.textDisabled} fontSize={9}>F</SvgText>
    </Svg>
  );
}

function FillupRow({ fillup, price }) {
  const [y, m, d] = fillup.date.split('-');
  const label = `${parseInt(d, 10)} ${MONTHS[parseInt(m, 10) - 1]} ${y}`;
  const perLit = fillup.liters > 0 ? (fillup.amount / fillup.liters).toFixed(1) : '—';
  ...
  return (
    <View style={styles.fillupRow}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.fillupSub}>{fillup.liters} L · RD${perLit}/L</Text>
      </View>
      <Text style={styles.fillupAmount}>RD${fillup.amount.toLocaleString()}</Text>
    </View>
  );
}
...
export function MiAutoScreen() {
  ...
  const [vehicle] = useState(VEHICLES[0]);
  const fillups   = SEED_FILLUPS;
  ...
  const costFull  = price > 0
    ? ((vehicle.tankSize / GALLON_TO_LITER) * price).toFixed(0)
    : '—';
  const costToFill = price > 0
    ? ((litToFill / GALLON_TO_LITER) * price).toFixed(0)
    : '—';
        ...
        {price > 0 && (
          <View style={styles.costRow}>
            <View style={styles.costCard}>
              ...
              <Text style={[styles.costValue, { color: colors.amber }]}>RD$ {Number(costFull).toLocaleString()}</Text>
        ...
        <View style={[styles.card, { overflow: 'hidden' }]}>
          {fillups.map((f, i) => (
            <React.Fragment key={i}>
              ...
              <FillupRow fillup={f} price={price} />
            </React.Fragment>
          ))}
        </View>
